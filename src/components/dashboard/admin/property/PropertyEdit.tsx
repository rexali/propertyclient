...
import { updatePropertyAPI } from '../../api/admin/property/updatePropertyAPI';
...
export default function PropertyAdd({ setOpen, propertyId }: { setOpen: any, propertyId: string }) {
    ...
    return (
        ...
        >
            <h2 className="text-2xl font-bold mb-4 text-gray-900 text-center flex justify-between">Edit Property <button onClick={() => setOpen(false)}><Forward /></button></h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    {property.images?.length && property.images.map(image => (<img key={image} crossOrigin='' src={`${BASE_URL_LOCAL + '/uploads/' + image}`} className='inline-block' alt={property.agent.name} />))}
                    <input
                        name="images"
                        type='file'
                        onChange={(e: any) => {
                ...
                <div>
                    ...
                    <img crossOrigin='' src={`${BASE_URL_LOCAL + '/uploads/' + property.agent.avatar}`} alt={property.agent.name} />
                    <input
                        name="avatar"
                        type='file'
                        onChange={(e: any) => {
                ...
                <div>
                    ...
                    <input
                        name="lng"
                        type='number'
                        value={Number(property.coordinates.lng)}
                        onChange={handleChange}
                        required
                        className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
